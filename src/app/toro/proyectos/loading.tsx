export default function ToroProjectsLoading() {
  return (
    <div className="space-y-4 pb-24 md:space-y-6 md:pb-8" aria-busy="true" aria-live="polite">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {Array.from({ length: 4 }).map((_, index) => (
          <div key={index} className="h-36 animate-pulse rounded-2xl border border-neutral-200 bg-white" />
        ))}
      </div>
      <span className="sr-only">Cargando proyectos de TORO.</span>
    </div>
  );
}
