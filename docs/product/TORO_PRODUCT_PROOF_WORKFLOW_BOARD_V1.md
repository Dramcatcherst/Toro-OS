...
|---|---|---|---|---|---|
| 1 | Executive daily brief / owner exceptions | TORO | Authenticated My TORO source-aware brief uses canonical executive decisions, active NOW/BLOCKED projects and a bounded sample of critical tasks. Production READY; baseline: docs/evidence/TORO_EXECUTIVE_BRIEF_READONLY_BASELINE_2026-09-23.md | RUNNING / READ-ONLY PROOF | Real canonical read path is implemented and served in production; next proof is owner use + evidence that the brief reduced search/decision effort. Do not mark VERIFIED from CI alone. |
...
| 3 | Shift / schedule request or change | TORO People | Existing shift templates/assignments/request tables + prepare-only schedule-change contract; evidence: docs/evidence/TORO_PEOPLE_SCHEDULE_CHANGE_PREPARE_2026-09-24.md | PREPARED / PREPARE-ONLY | Pure intent -> request payload is implemented/CI-tested. Next proof: authenticated employee RLS + one governed request insert + approval evidence. No shift mutation or employee rollout yet. |
...
| 8 | In-stay request / complaint -> service recovery | TERE + RICO + Comms | Existing guest handoff + canonical internal-work contract + prepare-only service-recovery adapter; evidence: docs/evidence/TORO_GUEST_SERVICE_RECOVERY_PREPARE_2026-09-24.md | PREPARED / PREPARE-ONLY | Guest issue -> internal-work + reply draft is implemented/CI-tested with no send/write. Next proof: isolated event -> one governed task -> operational evidence -> authorized follow-up, with replay dedupe and no shadow task. |
...
| 11 | System/connector incident -> recovery evidence | SOBRESITO + Systems | Systems Auditor + connector-health contracts + OpenClaw audit runbook; real PR #113 preview failure->fix->green recovery evidence | RUNNING / PREVIEW RECOVERY PROOF | Preview delivery recovery is evidenced; still need one real production-runtime or connector degradation/recovery cycle before VERIFIED. See docs/evidence/TORO_SYSTEMS_PREVIEW_RECOVERY_PROOF_2026-09-23.md |
