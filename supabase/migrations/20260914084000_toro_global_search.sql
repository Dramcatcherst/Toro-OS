-- TORO OS Phase 1: authorization-safe global search foundation.
-- Deliberately excludes finance, guests and private knowledge until those
-- domains have explicit search authorization contracts.
...
as $$
  ...
  room_results as (
    select
      ...
      '/toro/hotel?room=' || r.id::text as destination_path,
...
comment on function public.search_toro(text, integer) is
  'Authorization-safe TORO global search. Scoped to active org memberships; searches rooms, projects and non-private knowledge only. Finance/guest/private domains are intentionally excluded.';
