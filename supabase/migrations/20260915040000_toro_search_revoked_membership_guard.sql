-- TORO OS Phase 1 hardening: revoked org memberships must not retain search visibility.
-- Follow-up migration because the original global-search migration is already applied.
...
comment on function public.search_toro(text, integer) is
  'Authorization-safe TORO global search. Scoped only to active, non-revoked org memberships; searches rooms, projects and non-private knowledge. Finance/guest/private domains remain excluded.';
